  ...
  return (
    <div className="page-shell flex flex-col items-center gap-8 md:gap-12">
      ...
      >
        ...
        <h1 className="relative inline-block pb-5 text-3xl font-extrabold text-white after:absolute after:bottom-0 after:right-1/2 after:block after:w-20 after:translate-x-1/2 after:border-b-8 after:border-blue-600 md:text-6xl">
          Our Approach
        </h1>
      </div>

      <section className="w-full max-w-[70rem] space-y-4">
        <p className="text-sm font-semibold uppercase tracking-wide text-blue-900">Approaches</p>
        ...
        {APPROACHES.map((approach) => (
          <article key={approach.slug} className="flex h-full flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            ...
            <div className="flex flex-1 flex-col gap-3 p-5">
              <h3 className="text-lg font-bold text-blue-950 md:text-xl">{approach.title}</h3>
              <p className="flex-1 text-sm leading-6 text-slate-700">{approach.summary}</p>
              <Link
                href={`/our-approach/${approach.slug}`}
                className="inline-flex w-max rounded-3xl bg-blue-900 px-4 py-2 text-sm font-bold text-white transition hover:bg-blue-800"
              >
                Learn More
              </Link>
            </div>
